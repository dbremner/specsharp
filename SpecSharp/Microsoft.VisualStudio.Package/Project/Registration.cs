...
namespace Microsoft.VisualStudio.Package{
  ...
  public class RegistrationInfo{
    ...
    public virtual string GetResgenPath(){
      string version = this.GetCurrentVisualStudioVersion();
      string urtVersion = this.GetUrtVersion();
      if (version == null || urtVersion == null || urtVersion.Length < 4) return null;
      urtVersion = urtVersion.Substring(0, 4);
      char sep = Path.DirectorySeparatorChar;
      using (RegistryKey keySxS = Microsoft.Win32.Registry.LocalMachine.OpenSubKey(@"Software\Microsoft\VisualStudio\SxS\FRAMEWORKSDK")){
        if (keySxS != null){
          string path = keySxS.GetValue(version) as string;
          if (path != null && path.Length > 0) return path+sep+urtVersion+sep+"bin"+sep+"resgen.exe";
        }
      }
      string dir = this.GetCurrentVisualStudioInstallDir();
      if (dir != null) dir = Path.GetDirectoryName(Path.GetDirectoryName(dir));
      if (dir == null || dir.Length == 0)
        dir = Path.GetDirectoryName(typeof(object).Assembly.Location);
      return dir+sep+"resgen.exe";
    }
...
#else
      return "7.1"+Experimental;
#endif
    }
    public virtual string GetCurrentVisualStudioInstallDir(){
#if DEV11
      using (RegistryKey keyVS = Microsoft.Win32.Registry.LocalMachine.OpenSubKey ("Software\\Microsoft\\VisualStudio\\11.0")){
	      if (keyVS != null)
...
#elif WHIDBEY
      using (RegistryKey keyVS = Microsoft.Win32.Registry.LocalMachine.OpenSubKey ("Software\\Microsoft\\VisualStudio\\8.0")){
	      if (keyVS != null)
          return keyVS.GetValue("InstallDir") as string;
      }
#else
      using (RegistryKey keyVS = Microsoft.Win32.Registry.LocalMachine.OpenSubKey ("Software\\Microsoft\\VisualStudio\\7.1")){
	      if (keyVS != null)
          return keyVS.GetValue("InstallDir") as string;
      }
#endif
    ...
    public virtual string GetRegistrationScript(){
      string packageScript = GetResourceAsString(typeof(PackageInstaller), "Microsoft.VisualStudio.Package.Project.InstallPackage.rgs");
      string projectScript = GetResourceAsString(typeof(PackageInstaller), "Microsoft.VisualStudio.Package.Project.InstallProject.rgs");
      string languageScript = GetResourceAsString(typeof(PackageInstaller), "Microsoft.VisualStudio.Package.Project.InstallLanguage.rgs");
      string editorScript = GetResourceAsString(typeof(PackageInstaller), "Microsoft.VisualStudio.Package.Project.InstallEditor.rgs");
      string debuggerScript = GetResourceAsString(typeof(PackageInstaller), "Microsoft.VisualStudio.Package.Project.InstallDebugger.rgs");
      string vsipLicense = GetResourceAsString(typeof(PackageInstaller), "Microsoft.VisualStudio.Package.Project.InstallVSIPLicense.rgs");
    ...
    public virtual NameValueCollection GetRegistrationScriptArguments(){
      ...
      args.Add("ASSEMBLY", "Microsoft.VisualStudio.Package");
      args.Add("ASSEMBLYVERSION", GetVersionFromStrongName(typeof(Project).Assembly.FullName));      
      ...
      if (this.PackageType != null) {
        ...
        args.Add("PackageAssemblyLocation", this.PackageType.Assembly.Location);
      ...
      if (LanguageServiceType != null) {
        ...
        if (this.LanguageProgId != null)
          args.Add("LanguageProgId", this.LanguageProgId);
        else
          args.Add("LanguageProgId", this.LanguageName+"."+this.SourceFileExtension);
  ...
  public sealed class PackageInstaller {
    ...
    private string GetMachineConfigPath() {
      string dir = Path.GetDirectoryName(typeof(object).Assembly.Location);
      return dir + "\\CONFIG\\machine.config";
    }
